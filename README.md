...
More details are available at the [original repo](https://github.com/SebLague/Chess-Challenge). 

## How it works
The bot (in `Chess-Challenge/src/My Bot/MyBot.cs`) is based on an minimax search function. 
...
To fix this, there's a seperate function, `CaptureSearch`, which is called when the depth on the main search reaches `0`. 
...
- **Piece-square tables:** each piece type gets a bonus or penalty depending on which square it's on. However, because of the token limit of the challenge, these are crammed into a 6-bit numbers inside bitboards of `ulong`s and are decoded at runtime.
- **Game phase weighting:** there are two scores calculated: a midgame (`mg`) and an endgame (`eg`). These are blended together based on pieces left on the board (the `phase`). As the game progresses, the piece-square tables switch to engame, meaning that the king is more likely to come out.
